...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return { title: `${cat.icon} ${cat.name}提示词大全 | PromptHub`, description: cat.description };
}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: 12 }}>
          {promptList.map((prompt) => (
            <Link key={prompt.id} href={`/prompt/${prompt.id}`} style={{ background: "white", borderRadius: 12, padding: "16px 20px", border: "1px solid #e5e7eb", display: "flex", flexDirection: "column", gap: 8 }}>
              ...
              <p style={{ fontSize: "0.85rem", color: "#64748b", lineHeight: 1.5 }}>{prompt.useCase}</p>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginTop: "auto" }}>
                {prompt.tags.map((tag) => <span key={tag} style={{ fontSize: "0.7rem", padding: "2px 6px", borderRadius: 4, background: "#f1f5f9", color: "#64748b" }}>{tag}</span>)}
              </div>
            </Link>
          ))}
        </div>
