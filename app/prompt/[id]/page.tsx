...
export function generateMetadata({ params }: { params: { id: string } }) {
  ...
  return {
    title: `${prompt.title} - AI提示词 | PromptHub`,
    ...
    <div style={{ maxWidth: 800, margin: "0 auto", padding: "0 16px" }}>
      <div style={{ padding: "32px 0" }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 24 }}>
          {prompt.tags.map((tag) => (
            <Link key={tag} href={`/search?q=${encodeURIComponent(tag)}`} style={{ padding: "4px 10px", borderRadius: 6, background: "#f1f5f9", color: "#64748b", fontSize: "0.8rem" }}>
              #{tag}
            </Link>
          ))}
        </div>
        ...
        {/* 变量说明 */}
        <div style={{ background: "#fffbeb", borderRadius: 12, padding: "16px 20px", border: "1px solid #fde68a", marginBottom: 24 }}>
